...
@php
  ...
  $firstItem = (isset($items) && $items instanceof \Illuminate\Support\Collection && $items->count())
              ? $items->first() : null;
  ...
  $price = (int)(($firstItem['harga'] ?? $firstItem['price'] ?? null) ?? ($data['harga'] ?? $data['price'] ?? 0));
  ...
  <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
    ...
    <div class="lg:col-span-2 space-y-4">
      ...
      <div class="border rounded-md p-4">
        ...
        <div class="flex gap-4">
          <div class="w-32 h-32 border rounded flex items-center justify-center p-2 bg-gray-50 overflow-hidden">
            @if($imageUrl)
      ...
      <div class="border rounded-md p-4">
        ...
        <div class="space-y-3">
          <label class="flex items-center gap-2">
            <input type="radio" id="radio-store" name="pickup_method_choice" value="store" {{ $selected==='store' ? 'checked' : '' }}>
            <span class="font-medium">Ambil di Toko (Ongkir Rp 0)</span>
          </label>

          <label class="flex items-center gap-2">
            <input type="radio" id="radio-jnt" name="pickup_method_choice" value="jnt" {{ $selected==='jnt' ? 'checked' : '' }}>
    ...
    <div>
      <div class="border rounded-md p-4">
        ...
        <form id="checkout-form" method="POST" action="{{ route('user.checkout.store') }}" class="mt-4">
          @csrf
          ...
          <input type="hidden" name="customer_phone"  value="{{ $u->no_telp ?? $u->phone ?? '081234567890' }}">
...
@push('scripts')
  ...
  (function(){
    ...
    document.addEventListener('DOMContentLoaded', function () {
      ...
      if (!payBtn) { console.error('[CHECKOUT] ❌ #myButton tidak ditemukan'); return; }
      if (!form)   { console.error('[CHECKOUT] ❌ #checkout-form tidak ditemukan'); return; }
      ...
      // log cek klik
      payBtn.addEventListener('click', function(){ console.log('[CHECKOUT] tombol diklik'); });
      ...
      payBtn.addEventListener('click', async function (e) {
        ...
        if (typeof snap === 'undefined' || typeof snap.pay !== 'function') {
          console.warn('[CHECKOUT] snap.js belum ter-load; fallback VTWeb jika token didapat');
        }

        try {
          const resp = await fetch("{{ route('midtrans.create-snap-token') }}", {
            method: 'POST',
            headers: {
          ...
          if (!data || !data.token) {
            alert('Response tidak mengandung token. Cek controller create-snap-token.');
            return;
          }
      ...
      function recalc(method){
        ...
        const shipping = method === 'jnt' ? {{ $shipRate }} : 0;
      ...
      if (radioStore) radioStore.addEventListener('change', () => { pickupInp.value = 'store'; recalc('store'); });
      if (radioJnt)   radioJnt.addEventListener('change',   () => { pickupInp.value = 'jnt';   recalc('jnt');   });
