...
@php
    ...
    $rtCart = \Illuminate\Support\Facades\Route::has('keranjang.tambah')
        ? 'keranjang.tambah'
        : (\Illuminate\Support\Facades\Route::has('user.keranjang.tambah')
            ? 'user.keranjang.tambah'
            : null);
    $rtCheckout = \Illuminate\Support\Facades\Route::has('pesanan.store')
        ? 'user.checkout.store'
        : (\Illuminate\Support\Facades\Route::has('checkout.store')
            ? 'checkout.store'
            : null);
@endphp
            ...
            <div x-data="{ qty: 1 }">
                ...
                <div class="mt-2">
                    ...
                    @if (!empty($isPre))
                        <div class="mt-1 text-sm text-gray-600 flex items-center gap-2">
                            <span
                                class="inline-flex items-center px-2 py-0.5 rounded bg-amber-100 text-amber-700 text-xs font-medium">
                                Pre-Order
                            </span>
                ...
                @if (!empty($colors))
                    <div class="mt-5">
                        ...
                        <div class="flex flex-wrap gap-2" id="colorList">
                            @foreach ($colors as $c)
                                <button type="button" class="px-3 py-1.5 rounded border text-sm hover:bg-gray-50"
                ...
                <div class="mt-4">
                    ...
                    <div class="flex flex-wrap gap-2" id="sizeList">
                        <button type="button" class="px-4 py-2 rounded border text-sm hover:bg-gray-50 transition"
                    ...
                    <div id="sizeGuide" class="hidden mt-3 p-4 bg-blue-50 rounded-lg border border-blue-200">
                        ...
                        <div class="mt-3 text-xs text-blue-600">
                            💡 <strong>Tips:</strong> Ukur dengan meteran kain untuk hasil yang akurat. Jika ragu antara 2
                            ukuran, pilih yang lebih besar.
                        </div>
                    </div>
                </div>
                ...
                <div class="mt-6 flex flex-wrap gap-3">
                    ...
                    @if ($rtCart)
                        <form method="POST" action="{{ route('keranjang.store') }}">
                            ...
                            <button type="submit" formaction="{{ route('keranjang.tambah') }}"
